			...
			switch ($context) {
				...
				case 'read':
					if (!$this->is_readable($post))
						return new WP_Error("dianxiaomi_api_user_cannot_read_{$resource_name}", sprintf(__('You do not have permission to read this %s', 'dianxiaomi'), $resource_name), array('status' => 401));
					break;

				case 'edit':
					if (!$this->is_editable($post))
						return new WP_Error("dianxiaomi_api_user_cannot_edit_{$resource_name}", sprintf(__('You do not have permission to edit this %s', 'dianxiaomi'), $resource_name), array('status' => 401));
					break;

				case 'delete':
					if (!$this->is_deletable($post))
						return new WP_Error("dianxiaomi_api_user_cannot_delete_{$resource_name}", sprintf(__('You do not have permission to delete this %s', 'dianxiaomi'), $resource_name), array('status' => 401));
					break;
			}
		}
